...
const UpdateIndicatorForm = ({ open, onClose, indicator, onIndicatorUpdated }) => {
  ...
  const { getUserTriggersRequest, error, loading } = useUserService();
  ...
  const loadTriggers = async (currentPage, initialTrigger = null) => {
    ...
    try {
      ...
      setMessage(t('errors.triggers.fetching_triggers', { error: err.message || err.toString() }));
  ...
  const handleSubmit = async () => {
    try {
      ...
      setMessage(t('indicator.success.update', { message: response.message }));
      onIndicatorUpdated?.();
      handleOnClose();
    } catch (err) {
      setMessage(t('indicator.errors.update', { error: err.message || err.toString() }));
